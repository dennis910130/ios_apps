...
class LGChatController : UIViewController, UITableViewDelegate, UITableViewDataSource, LGChatInputDelegate, NSStreamDelegate {
    ...
    */
            ...
            if error == nil {
                if let results = results {
                    for result in results {
                        let fromId = result["fromId"] as! String
                        if fromId == PFUser.currentUser()!.objectId {
                            let tmp:LGChatMessage = LGChatMessage(content: result["text"] as! String, sentByString: "LGChatMessageSentByUser")
                            self.addNewMessage(tmp)
                        } else {
                            let tmp:LGChatMessage = LGChatMessage(content: result["text"] as! String, sentByString: "LGChatMessageSentByOpponent")
                            self.addNewMessage(tmp)
                        }
                    }
                }
    ...
    private func setupLogOutButton() {
        logOutButton = UIBarButtonItem(title: "Log Out", style: UIBarButtonItemStyle.Plain, target: self, action: Selector("logOut"))
    ...
    func chatInput(chatInput: LGChatInput, didSendMessage message: String) {
        ...
        if shouldSendMessage {
            ...
            msg["fromId"] = PFUser.currentUser()!.objectId!
            msg["toId"] = PFUser.currentUser()!["couple"]!
            msg["text"] = message
            msg["coupleId"] = PFUser.currentUser()!["coupleId"]!
            msg["time"] = NSDate()
            //msg.setObject(NSDate(), forKey: "time")
